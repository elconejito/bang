  ...
  <div class="mx-auto max-w-[1280px] px-8 py-6 pb-16">
    ...
    >
      <template v-if="!loading && lowCount > 0" #count-suffix>
        <span
          class="inline-flex items-center gap-1 rounded border border-[#e0a999] bg-[#f7e9e4] px-2 py-px font-mono text-[11px] text-[#b4452f]"
        >
          <svg
            class="h-[11px] w-[11px]"
            viewBox="0 0 24 24"
      ...
      <div v-for="n in 2" :key="n" class="mb-8">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div
            v-for="i in 3"
            :key="i"
            class="h-[148px] animate-pulse rounded border border-line bg-white"
          />
        </div>
      </div>
    </template>
