...
const OPTIC_TYPE_LABELS = { red_dot: 'Red dot', prism: 'Prism', lpvo: 'LPVO', variable: 'Variable' };
    ...
    <template v-else-if="optic">
      <div class="flex items-center gap-4 mb-6 flex-wrap">
        <div class="flex-1 min-w-0">
          <h1 class="font-display font-bold text-[28px] tracking-[-0.02em] mb-1">{{ optic.label }}</h1>
          <div class="text-[15px] text-[#6b7077]">{{ optic.manufacturer }} · Optic<template v-if="optic.optic_type"> · {{ OPTIC_TYPE_LABELS[optic.optic_type] ?? optic.optic_type }}</template></div>
        </div>
        <div class="flex items-center gap-2.5">
          <router-link :to="{ name: 'OpticEdit', params: { optic_id: optic.id } }" class="inline-flex items-center gap-1.5 bg-white text-[#1a1c1f] font-semibold text-[14px] px-[14px] py-2 rounded border border-[#c2c6ca] hover:bg-[#f5f6f7] transition-colors">
      ...
      <div class="grid grid-cols-[344px_1fr] gap-6 items-start">
        <div class="flex flex-col gap-4">
          ...
          <div class="overflow-hidden rounded border border-line bg-surface">
            <router-link :to="{ name: 'OpticGallery', params: { optic_id: opticId } }" class="block">
          ...
          <component
            ...
            :to="optic.firearm_id ? { name: 'FirearmsShow', params: { firearm_id: optic.firearm_id } } : undefined"
            :class="optic.firearm_id ? 'bg-[#e7f1eb] border-[#9ccbb1] hover:bg-[#e0ede6]' : 'bg-[#f5f6f7] border-[#c2c6ca]'"
            class="border rounded-sm px-4 py-[13px] flex items-center gap-3 transition-colors"
          >
            <div :class="optic.firearm_id ? 'border-[#9ccbb1] text-[#2f7d57]' : 'border-[#c2c6ca] text-[#5b6066]'" class="w-9 h-9 rounded-sm bg-white border flex items-center justify-center flex-none">
            ...
            <div class="flex-1 min-w-0">
              <div :class="optic.firearm_id ? 'text-[#2f7d57]' : 'text-[#5b6066]'" class="font-mono text-[10px] tracking-[0.06em]">{{ optic.firearm_id ? 'MOUNTED ON' : 'UNMOUNTED' }}</div>
              <div class="text-[16px] font-semibold truncate">{{ optic.firearm ? (optic.firearm.label ?? optic.firearm.manufacturer) : (optic.location?.label ?? 'No location') }}</div>
              <div v-if="optic.firearm" class="text-[12px] text-[#5b7466] truncate">
                <template v-if="optic.firearm.label && optic.firearm.manufacturer">{{ optic.firearm.manufacturer }}</template>
                <template v-if="optic.mounted_since">{{ (optic.firearm.label && optic.firearm.manufacturer) ? ' · ' : '' }}since {{ dayjs(optic.mounted_since).format('MMM D') }}</template>
              </div>
            </div>
          ...
          <div class="bg-white border border-[#e2e4e6] rounded-sm overflow-hidden">
            ...
            <div class="px-4 py-1.5">
              <div v-if="optic.optic_type" class="flex items-center justify-between py-[9px] border-b border-[#f1f2f3]"><span class="text-[14px] text-[#6b7077]">Type</span><span class="text-[14px]">{{ OPTIC_TYPE_LABELS[optic.optic_type] ?? optic.optic_type }}</span></div>
              <div v-if="optic.battery_type" class="flex items-center justify-between py-[9px] border-b border-[#f1f2f3]"><span class="text-[14px] text-[#6b7077]">Battery</span><span class="text-[14px]">{{ optic.battery_type }}</span></div>
              <div v-if="optic.serial" class="flex items-center justify-between py-[9px] border-b border-[#f1f2f3]"><span class="text-[14px] text-[#6b7077]">Serial #</span><span class="font-mono text-[14px]">{{ optic.serial }}</span></div>
              <div v-if="optic.purchase_date" class="flex items-center justify-between py-[9px]"><span class="text-[14px] text-[#6b7077]">Purchased</span><span class="text-[14px]">{{ dayjs(optic.purchase_date).format('MMM YYYY') }}<template v-if="optic.purchase_price"> · <span class="font-mono">${{ Number(optic.purchase_price).toLocaleString() }}</span></template></span></div>
            </div>
          </div>
        </div>
        <AccessoryEventTimeline
          ...
          :manual-event-types="[{ value: 'BATTERY_REPLACE', label: 'Battery replacement' }, { value: 'REPAIR', label: 'Repair / Service' }]"
        />
      </div>
    </template>
