...
onMounted(async () => {
  const [firearmRes, picturesRes] = await Promise.allSettled([
    firearmsStore.fetchOne(props.firearmId),
    picturesStore.fetchForFirearm(props.firearmId),
  ])
  if (firearmRes.status === 'fulfilled') firearm.value = firearmRes.value.data
  if (picturesRes.status === 'fulfilled') pictures.value = picturesRes.value.data
  loading.value = false
})
...
const crumbs = computed(() => [
  ...
  { label: firearm.value?.label ?? firearm.value?.manufacturer ?? '…', to: { name: 'FirearmsShow', params: { firearm_id: props.firearmId } } },
  { label: 'Photos' },
])
...
async function reloadPictures() {
  const { data } = await picturesStore.fetchForFirearm(props.firearmId)
  pictures.value = data
}
...
async function onFileSelected(e) {
  ...
  try {
    for (const file of files) {
      await picturesStore.uploadToFirearm(props.firearmId, file)
    }
...
async function setPrimary(pic) {
  await picturesStore.setPrimary(props.firearmId, pic.id)
  pictures.value = pictures.value.map((p) => ({ ...p, is_primary: p.id === pic.id }))
}

// Detach
async function detach(pic) {
  await picturesStore.detachFromFirearm(props.firearmId, pic.id)
  pictures.value = pictures.value.filter((p) => p.id !== pic.id)
  // If we removed the primary and others remain, mark first as primary locally
...
async function onDrop(e, index) {
  ...
  dragSrcIndex.value = null
  dragOverIndex.value = null
  await picturesStore.reorder(props.firearmId, reordered.map((p) => p.id))
}
...
<template>
  <div class="max-w-[1080px] mx-auto px-8 py-6 pb-16">
    ...
    <p class="text-[15px] text-[#6b7077] mb-5">
      Attached to <span class="font-semibold text-[#3a3e44]">{{ firearm?.label ?? firearm?.manufacturer }}</span>.
      ...
      <div class="grid grid-cols-4 gap-3.5">
        ...
        >
          ...
          <div
            ...
            class="absolute top-2 left-2 inline-flex items-center gap-1 bg-[rgba(26,28,31,0.78)] text-white rounded-sm px-[7px] py-px text-[11px]"
            :class="pic.is_primary ? 'top-8' : 'top-2'"
          >
          ...
          <div class="absolute bottom-2 right-2 flex gap-1.5">
            <!-- Set primary (only for non-primary) -->
            <button
              v-if="!pic.is_primary"
