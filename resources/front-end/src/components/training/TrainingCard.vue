...
const locationLabel = computed(() => props.session.range?.label ?? props.session.location?.label ?? '');
...
<template>
  ...
  >
    <!-- Date rail -->
    <div class="flex min-w-[70px] flex-col items-center justify-center border-r border-[#eef0f1] bg-[#fafbfb] px-4 py-3 text-center">
      <span class="font-mono text-[10px] uppercase tracking-[0.05em] text-muted">{{ date.format('ddd') }}</span>
      <span class="my-0.5 font-mono text-[24px] font-semibold leading-none text-ink-900">{{ date.format('D') }}</span>
    ...
    <div class="min-w-0 flex-1 px-4 py-3">
      <div class="flex items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
          <div class="font-display text-[18px] font-semibold leading-[1.15] text-ink-900 transition-colors group-hover:text-brass-800">
            {{ session.label }}
          </div>
          <div v-if="locationLabel" class="mt-0.5 inline-flex items-center gap-1.5 text-[13px] text-[#6b7077]">
        ...
        <div class="hidden shrink-0 items-start gap-5 text-right sm:flex">
          <div>
            <div class="font-mono text-[20px] font-medium leading-none">{{ session.total_rounds.toLocaleString() }}</div>
            <div class="mt-1 font-mono text-[9px] tracking-[0.05em] text-muted">ROUNDS</div>
          </div>
          <div>
            <div class="font-mono text-[20px] font-medium leading-none" :class="targetCount ? 'text-ink-900' : 'text-ink-300'">
              {{ targetCount || '-' }}
            </div>
      ...
      <!-- Firearm chips -->
      <div v-if="session.firearms_used?.length || session.has_suppressor" class="mt-3 flex flex-wrap gap-1.5">
        <span
