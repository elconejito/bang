...
const form = ref({
  event_type: props.availableTypes[0]?.value ?? '',
  event_date: today,
  description: '',
});
...
<template>
  ...
  >
    ...
    >
      ...
      <div class="px-[18px] py-5 flex flex-col gap-4">
        ...
        <div>
          <label class="block text-[13px] font-semibold text-[#3a3e44] mb-1.5"
            >Notes <span class="font-normal text-[#8a9098]">(optional)</span></label
          >
          <textarea
      ...
      <!-- Footer -->
      <div
        class="flex items-center justify-end gap-2.5 px-[18px] py-3.5 border-t border-[#eef0f1] bg-[#fafbfb]"
      >
        <button
