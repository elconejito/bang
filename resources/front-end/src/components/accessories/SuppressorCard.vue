  ...
  >
    <div class="p-[14px_16px] flex flex-col gap-2 flex-1">
      <div class="flex items-center justify-between">
        <span class="font-mono text-[9px] tracking-[0.06em] text-[#8a9098] border border-[#d6d9dc] rounded-sm px-[7px] py-[2px] flex items-center gap-1.5">
          SUPPRESSOR
          <span v-if="suppressor.is_nfa" class="text-white bg-[#1a1c1f] rounded-[2px] px-1">NFA</span>
        </span>
      ...
      <div class="flex gap-1.5 flex-wrap mt-auto">
        <span v-if="suppressor.caliber" class="text-[12px] border border-[#c2c6ca] rounded-sm px-[9px] py-[1px] text-[#3a3e44] bg-[#f5f6f7]">
          {{ suppressor.caliber.label }}
        </span>
        <span v-if="suppressor.mount_type" class="text-[12px] border border-[#c2c6ca] rounded-sm px-[9px] py-[1px] text-[#3a3e44] bg-[#f5f6f7]">
          {{ suppressor.mount_type }}
        </span>
      </div>
    </div>
    <div class="border-t border-[#eef0f1] flex items-center justify-between px-4 py-[9px] bg-[#fafbfb]">
      <span v-if="suppressor.firearm" class="font-mono text-[10px] text-[#2f7d57] border border-[#9ccbb1] bg-[#e7f1eb] rounded-sm px-[7px] py-[2px] flex items-center gap-1.5">
