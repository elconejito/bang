...
<template>
  <div class="fixed inset-0 bg-[rgba(20,22,26,0.46)] flex items-start justify-center p-12 z-40 overflow-auto" @click.self="emit('close')">
    <div class="w-[720px] max-w-full bg-white border border-[#d6d9dc] rounded shadow-[0_10px_30px_rgba(20,22,26,0.22)] overflow-hidden">
      <!-- Header -->
      ...
      <div class="p-[18px] grid grid-cols-5 gap-2.5 max-h-[360px] overflow-y-auto">
        <div v-if="loading" class="col-span-5 py-8 text-center text-[14px] text-muted">Loading…</div>
        <div v-else-if="!library.length" class="col-span-5 py-8 text-center text-[14px] text-muted">No photos in your library yet.</div>

        <div
          v-for="pic in library"
          :key="pic.id"
          class="relative rounded overflow-hidden cursor-pointer"
          :class="isAttached(pic.id)
        ...
        >
          ...
          <div v-if="isAttached(pic.id)" class="absolute inset-0 flex items-end justify-center pb-1.5">
            <span class="font-mono text-[9px] text-white bg-[rgba(26,28,31,0.8)] rounded-sm px-1.5 py-px">ATTACHED</span>
          </div>
          ...
          >
            <svg class="w-3 h-3 text-[#1a1c1f]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>
          </div>
