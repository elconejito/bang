...
class MagazineGroupController extends Controller
{
    public function bulkUpdate(BulkUpdateMagazineGroupRequest $request, int $group, BulkUpdateMagazines $action): JsonResponse
    {
        ...
        return response()->json([
            ...
            'meta' => [
                ...
                'remaining_group' => $result['remaining_group'],
                'updated_group' => $result['updated_group'],
            ],
        ]);
    }
    ...
    {
        return ['key' => $groupId, 'manufacturer' => $magazine->manufacturer, 'model_name' => $magazine->model_name, 'model_number' => $magazine->model_number, 'capacity' => $magazine->capacity, 'calibers' => $magazine->calibers->map->only(['id', 'label'])->values()->all()];
    }

    private function magazineRow(Magazine $magazine): array
    {
        return ['id' => $magazine->id, 'label' => $magazine->label, 'model_number' => $magazine->model_number, 'serial_number' => $magazine->serial_number, 'id_marking' => $magazine->id_marking, 'color_id' => $magazine->color_id, 'color' => $magazine->color?->only(['id', 'label']), 'calibers' => $magazine->calibers->map->only(['id', 'label'])->values()->all(), 'display_status' => $magazine->display_status, 'lifecycle_status' => $magazine->isArchived() ? 'archived' : 'active', 'archived_at' => $magazine->archived_at?->toISOString(), 'archive_reason' => $magazine->archive_reason?->value, 'archive_description' => $magazine->archive_description, 'load_state' => $magazine->load_state, 'loaded_rounds' => $magazine->loaded_rounds, 'capacity' => $magazine->capacity, 'loaded_ammunition' => $magazine->loadedAmmunition?->only(['id', 'manufacturer', 'label']), 'current_firearm' => $magazine->currentFirearm?->only(['id', 'label', 'manufacturer']), 'compatible_firearms' => $magazine->compatibleFirearms->map->only(['id', 'label', 'manufacturer'])->values()->all(), 'location' => $magazine->location?->only(['id', 'label'])];
    }
}
